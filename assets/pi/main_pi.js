...
function connectElgatoStreamDeckSocket(
  ...
  websocket.onmessage = function (evt) {
    ...
    if (jsonObj.event === "didReceiveSettings") {
      ...
      initiateElement("frequency", payload.frequency, 600000);
      initiateElement("unit", payload.unit, "celsius");
      initiateElement("roundDegree", String(payload.roundDegree), "true");
    }
...
function updateSettings() {
  if (websocket && websocket.readyState === 1) {
    ...
    payload.roundDegree = document.getElementById("roundDegree").value === "true";
    const json = {
      event: "setSettings",
      context: uuid,
      payload: payload,
